...
bool AgendaService::resetInfo(const std::string &userName,
	...
	if (email == "" && phone == "") {
		...
		list<User> temp = m_storage->queryUser([&](const User &u) {return (u.getName() == userName); });
		if (temp.size() != 1) {
			return 0;
		}
		else {
			int count = m_storage->updateUser(
				[&](const User &u) {
...
int AgendaService::resetPsd(const std::string &userName,
	const std::string &old_password,
	const std::string &new_password) {
	if (new_password.size() < 9 || new_password.size() > 20) {
		return 3;
	}
	list<User> temp = m_storage->queryUser([&](const User &u) {
		return (u.getName() == userName && u.getPassword() == old_password);
	});
	if (temp.size() != 1) {
		return 2;
	}
